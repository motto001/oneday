     ...
     } else
     {
         ...
         if (!empty($_FILES) ) {
             ...
             if (in_array($fileParts['extension'],$kep_tipusok)) {
                 ...
                 $filenev=$manipulator->webnev(substr($fileParts['filename'], 0,20).'.'.$fileParts['extension']);
                 $filenev=$manipulator->vanfile_plusz($targetFolder.'/'.$filenev);
                 //$filenev=$_FILES[$feltolt_mezo]['name']; 
                 $manipulator->save($filenev);
 // echo $targetFolder.'/'.$filenev;
                // $newImage = $manipulator->resample(100,100);
                 $manipulator->save(\lib\base\LINK::thumb_src($filenev));
